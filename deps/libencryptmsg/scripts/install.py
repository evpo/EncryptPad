...
def main(args):
    ...
    build_include_dir = os.path.join(options.build_dir, 'include', 'encryptmsg')
    ...
    if 'encryptmsg_pkgconfig' in cfg:
        pkgconfig_dir = os.path.join(options.prefix, options.libdir, options.pkgconfigdir)
        makedirs(prepend_destdir(pkgconfig_dir))
        copy_file(cfg['encryptmsg_pkgconfig'],
                  prepend_destdir(os.path.join(pkgconfig_dir, os.path.basename(cfg['encryptmsg_pkgconfig']))))
    ...
    logging.info('EncryptMsg %s installation complete', cfg['version'])
    return 0
