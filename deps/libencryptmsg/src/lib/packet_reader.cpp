...
class CompressedRW : public PacketRWBase
{
    ...
    private:
        ...
        std::unique_ptr<Bzip2Session> bzip2_session_;
};
            ...
            :PacketRWBase(state, false) // is_final_packet = false
{
    ...
    cipher_mode_ = Botan::Cipher_Mode::create(algo_spec.botan_name, Botan::Cipher_Dir::Decryption);
...
void CompressedRW::ReadAlgo()
{
    ...
    switch(state_.message_config.GetCompression())
    {
        ...
        case Compression::ZLIB:
            decompression_ = Botan::Decompression_Algorithm::create(
                    GetCompressionSpec(state_.message_config.GetCompression()).botan_name);
            if(decompression_)
                decompression_->start();
            break;
        case Compression::BZip2:
            bzip2_session_ = std::make_unique<Bzip2Session>(state_.output_buffer_size);
            if(bzip2_session_->InitFailed())
            {
                bzip2_session_.reset();
                state_.emsg_result = EmsgResult::UnexpectedError;
            }
            break;
        default:
...
{
    ReadAlgo();
    if(state_.message_config.GetCompression() == Compression::BZip2)
    {
        if(!bzip2_session_)
            return EmsgResult::UnexpectedError;
        auto result = bzip2_session_->Read(in_, out);
        switch(result)
        {
            ...
            case Bzip2SessionResult::OutputBufferOverflow:
                state_.output_buffer_overflow = true;
                return EmsgResult::Pending;
            case Bzip2SessionResult::Error:
            default:
                return EmsgResult::UnexpectedError;
        }
    }
